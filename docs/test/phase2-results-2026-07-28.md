...
| P2-K4 | FAIL | 10 concurrent GETs same dept: codes=[200], min=7.01s max=9.81s (no 5xx, no deadlock) — max EXCEEDS the 3s threshold (~7x degradation vs single-user p50; measurement includes 10 fresh TLS+EasyAuth handshakes from one client machine, but server-side queueing on the single warm replica is the likely dominant factor) |
