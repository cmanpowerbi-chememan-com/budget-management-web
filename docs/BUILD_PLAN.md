...
**A11. Scheduled jobs** (GitHub Actions cron like sync_employees, OR Azure Function timer): **auto-submit** DRAFT ฝ่าย at deadline (approver1=last-editor's manager, log AUTO_SUBMIT). Use ONLY the current planning year's `submission_deadline`. **[RESOLVED 2026-07-16, jakkaritw]: DEFERRED to Phase 2** — ADR-0006's admin "stuck/overdue approvals view + departed-approver reassign (ADMIN_OVERRIDE)" is out of Phase-1. **[SUPERSEDED 2026-08-01, ADR-0027]:** this section originally covered the stuck-step case with a 30-day auto-escalate job — that job is now DELETED outright (not deferred), replaced by a manual **admin step-override** (`POST /approval/override-step`, position-1 only, no waiting period, logs `ADMIN_STEP_OVERRIDE`); see ADR-0027 for the full rationale. Same decision round: final-APPROVED confirmation email to the submitter = IN (built as an A12 follow-up); month cells = positive amounts only (no negative/reversal entry).
