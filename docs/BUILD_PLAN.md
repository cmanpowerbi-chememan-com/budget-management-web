...
**Hard dependency on the DATA project:** app RLS/approval/read **cannot run until** `dbo.cc_filler_map` + the confirmed employee source (DATA #2/#5) and `budget.board_budget`/`submission_deadline` (DATA #6/#8) are live. Get the object names + connection strings + the confirmed `gold.fact_gl_trans` refresh owner from the data team first.
...
**A2. Scaffold `backend/` + Entra Easy Auth.** `x-ms-client-principal-name` header = login email (trust anchor, ADR-0004). Connection helpers (split-connection; never one cross-store JOIN): app Fabric SQL DB (`budget.*`), DW `fabric_sql_database` (masters + employee), DW gold warehouse (SAP). CC existence/name at write = from `dbo.cc_filler_map` + the DW GL-name ref (DATA #4) — NOT the retiring app Lakehouse.

**A3. RLS resolution** (ADR-0019, `project_primary_manager_rule`): Fill = `cc_filler_map WHERE filler_email=@me`; See = Fill ∪ (CCs where @me = a Filler's Primary-row managerempcode; Acting ignored) — ONE in-DB JOIN (cc_filler_map + employee same DB). Filler-not-in-master → still Fills, no See-manager, approver1→Nipaporn. Admin overlay = ADMIN_EMAILS (before membership; toggle-gated, ADR-0012/0014); jakkaritw = external full admin. RED-first tests: scope resolution incl thanakorny-not-dropped.

**A4. Main-table read** (ADR-0010/0020): union on `(cost_center, gl_account)` — `board(fy=Y)` LEFT JOIN `pending(fy=Y+1)` in app DB; SAP read-through (DATA #7 query) merged in FastAPI by (cc,gl); RLS-filtered. Approved-Y=reference; SAP-led (cc,gl) with no pending → show editable blank Pending-Y+1 row. RED-first: SAP SUM/sign/filter parity.
