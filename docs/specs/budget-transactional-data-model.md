...
| **`dbo.v_employee_primary`** — **VIEW (read-only), fallback / full-roster source (NOT for RLS/approval)** | `dbo` (Fabric SQL DB, DW `cman-dw-ws`, next to `employee_master` — ADR-0022) | `employee_code`, `email`, `full_name_th`, `job_level_name_en`, `manager_employee_code`, `manager_email`, `position_status`, `org_code`, … (~68 cols total) | Primary-row dedup of the unfiltered 649-row `employee_master`, **612 rows, one row per person**. Base of the trip-subform traveler picker (`budget_trip.traveler_empcode`) because a traveler may be outside the 497-row budget filter that `v_employee_budget_01` applies. |
| **`dbo.v_employee_orgmap` + `dbo.v_traveler_picker`** — **VIEWs (read-only), trip-subform traveler picker source (live-verified 2026-07-19)** | `dbo` (same DB) | orgmap: `employee_code`, `email` (lowered), `dept_key` (= `LEFT(org_code,4)` + suffix-stripped org name), `dept_name`, `division_key`, `mgr1_*`/`mgr2_*` resolved; picker: `filler_email`, `traveler_empcode/name/position/email`, `pick_reason` | DDL in `db/ddl/traveler_picker_views.sql`, built over `v_employee_primary`. Picker grain = filler_email × traveler: same-department UNION direct manager + manager's manager UNION direct reports, deduped one row per traveler. Dept key derives from ONE source (no cross-system name matching — the cc_filler_map↔org_name match fails for ~70% of ฝ่าย). Live validation vs all 99 distinct `cc_filler_map.filler_email`: 99/99 resolve, mgr chain 100%, picker size min=3 / median=7 / max=46. `fetch_travelers` falls back to the full `v_employee_primary` roster for logins absent from the roster (admin/test accounts). |
...
- **Employee source CONFIRMED (real-data verified 2026-07-15 against live `dbo` in
  ...
  dedup of unfiltered `employee_master`) for callers needing the full roster — since 2026-07-19
  it is also the base of `v_employee_orgmap`/`v_traveler_picker`, which feed the trip-subform
  traveler picker dept-scoped per login (see the table above; full-roster fallback only for
  logins absent from the roster).
...
Table dbo.submission_deadline {
  ...
  reminder_date date         [not null]   // = deadline_date MINUS reminder_day days (reminder_day is a COUNT OF DAYS BEFORE, not a day-of-month — corrected 2026-08-18, DW commit 7f51775); the reminder email fires on this date
  _user         nvarchar(150)[not null]
  _updated_at   datetime2    [not null]
}
```
...
| cost_center (all owned tables) | No | — | Must exist in gold_sap_m_cost_center AND not excluded | Validate by EXISTENCE in master, never by length (short codes PBAW01 exist). Excluded CCs: CMRY01, CMKK01, CMPB01, MNLB00..04 — reject for budget entry. (**10SC012000 removed from the exclusion list 2026-07-14** — it is a valid CC for BOTH budget entry and SAP actuals; same list as the ADR-0020 SAP filter, kept in sync.) |
...
| submission_deadline | No | Yes (fiscal_year PK) | one row per year | Source = SharePoint `วันปิดรับข้อมูลงบประมาณ.xlsx` (5 cols: `fiscal year, closing date, closing month, closing year, reminder_day` — all TEXT, sync casts to int). Derive `deadline_date = DATE(closing_year, closing_month, closing_date)` and `reminder_date = deadline_date - reminder_day days` — **`reminder_day` is a COUNT OF DAYS BEFORE the deadline, not a day-of-month** (corrected 2026-08-18 in DW commit 7f51775; the old day-of-month reading put FY2027's reminder on 2026-10-15 for a 2026-10-07 deadline, so the window opened 8 days after it closed and no reminder was ever sent). Validate `reminder_date < deadline_date`, `reminder_day` within 1..365 and `closing_year` within 2000..2100 — the range checks run BEFORE any date construction so a stray cell cannot abort the whole file; `reminder_day > 60` warns but still loads. The old `reminder_day < closing_date` check encoded the wrong meaning and never fired. The transform names it `reminder_days_before` internally; **the DB column is still `reminder_day`** and the app's SELECTs are unaffected. Never patch this table with a direct SQL UPDATE — the daily sync overwrites it from the Excel master. After deadline_date — AND for ALL past fiscal years once the cycle rolls forward to the next planning year (**year rollover, confirmed 2026-07-14**): user Pending (pending_budget) writes are LOCKED = **read-only for Fillers/users**. **Admin can still edit ANY Pending freely — no approval step, status unchanged (ADR-0012 / ADR-0013) — including past-year Pending.** board_budget import NOT locked. Auto-submit/reminder automation uses ONLY the current planning year's row (past-year rows may be placeholder). **Reminder email** (fires on `reminder_date`, Phase-1): sent to the Fillers of ฝ่าย still in DRAFT (NOT yet submitted) for the planning year — one email per Filler listing THEIR still-pending ฝ่าย (a Filler spanning many ฝ่าย gets ONE grouped email; already-submitted ฝ่าย are excluded, no spam); via Graph sendMail (`send_signoff_email.py` pattern). |
