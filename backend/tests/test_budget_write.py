...
# ---------------------------------------------------------------------------
# DELETE /budget/rows (grid trailing "ลบ" column)
# ---------------------------------------------------------------------------

def test_delete_row_401_without_auth(client):
    response = client.delete(
        "/budget/rows",
        params={"cost_center": "CC1", "gl_account": "GL1", "fiscal_year": 2027, "expected_updated_at": "2026-01-01T00:00:00Z"},
    )
    assert response.status_code == 401


def test_delete_row_success_returns_200(client):
    from app.write_model import RowDeleteResult

    _override_auth("filler@chememan.com")
    fake_result = RowDeleteResult(cost_center="CC1", gl_account="GL1", fiscal_year=2027, ok=True)
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
        "app.routers.budget_write.resolve_scope", return_value=_fake_scope()
    ), patch("app.routers.budget_write.delete_pending_row", return_value=fake_result):
        mock_conn.return_value.__enter__.return_value = MagicMock()
        response = client.delete(
            "/budget/rows",
            params={"cost_center": "CC1", "gl_account": "GL1", "fiscal_year": 2027, "expected_updated_at": "2026-01-01T00:00:00Z"},
...
def test_delete_row_forbidden_maps_to_403(client):
    from app.write_model import RowDeleteResult

    _override_auth("outsider@chememan.com")
    fake_result = RowDeleteResult(
        cost_center="CC1", gl_account="GL1", fiscal_year=2027, ok=False,
        error="forbidden", detail="CC1 is not in your Fill scope",
    )
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
...
def test_delete_row_conflict_maps_to_409(client):
    from app.write_model import RowDeleteResult

    _override_auth("filler@chememan.com")
    fake_result = RowDeleteResult(
        cost_center="CC1", gl_account="GL1", fiscal_year=2027, ok=False,
        error="conflict", detail="CC1/GL1/2027 was changed by someone else — reload and retry",
    )
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
        "app.routers.budget_write.resolve_scope", return_value=_fake_scope()
    ), patch("app.routers.budget_write.delete_pending_row", return_value=fake_result):
...
def test_delete_row_missing_query_params_is_422(client):
    _override_auth("filler@chememan.com")
    response = client.delete("/budget/rows")
    assert response.status_code == 422


# ---------------------------------------------------------------------------
# DELETE /budget/detail, DELETE /budget/trip (A9 backend gap close)
# ---------------------------------------------------------------------------

def test_delete_detail_401_without_auth(client):
    response = client.delete(
        "/budget/detail", params={"detail_id": 5, "expected_updated_at": "2026-01-01T00:00:00Z"}
    )
    assert response.status_code == 401


def test_delete_detail_success_returns_200(client):
    from app.write_model import DetailLineDeleteResult

    _override_auth("filler@chememan.com")
    fake_result = DetailLineDeleteResult(detail_id=5, ok=True)
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
        "app.routers.budget_write.resolve_scope", return_value=_fake_scope()
    ), patch("app.routers.budget_write.delete_detail_line", return_value=fake_result):
        mock_conn.return_value.__enter__.return_value = MagicMock()
        response = client.delete(
            "/budget/detail", params={"detail_id": 5, "expected_updated_at": "2026-01-01T00:00:00Z"}
        )
...
def test_delete_detail_forbidden_maps_to_403(client):
    from app.write_model import DetailLineDeleteResult

    _override_auth("outsider@chememan.com")
    fake_result = DetailLineDeleteResult(detail_id=5, ok=False, error="forbidden", detail="CC1 is not in your Fill scope")
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
        "app.routers.budget_write.resolve_scope", return_value=_fake_scope(fill_cost_centers=[], see_cost_centers=[])
    ), patch("app.routers.budget_write.delete_detail_line", return_value=fake_result):
        mock_conn.return_value.__enter__.return_value = MagicMock()
...
def test_delete_detail_conflict_maps_to_409(client):
    from app.write_model import DetailLineDeleteResult

    _override_auth("filler@chememan.com")
    fake_result = DetailLineDeleteResult(detail_id=5, ok=False, error="conflict", detail="detail line changed by someone else")
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
        "app.routers.budget_write.resolve_scope", return_value=_fake_scope()
    ), patch("app.routers.budget_write.delete_detail_line", return_value=fake_result):
        mock_conn.return_value.__enter__.return_value = MagicMock()
...
def test_delete_detail_missing_query_params_is_422(client):
    _override_auth("filler@chememan.com")
    response = client.delete("/budget/detail")
    assert response.status_code == 422


def test_delete_trip_401_without_auth(client):
    response = client.delete("/budget/trip", params={"trip_id": 7, "expected_updated_at": "2026-01-01T00:00:00Z"})
    assert response.status_code == 401


def test_delete_trip_success_returns_200(client):
    from app.write_model import TripDeleteResult

    _override_auth("filler@chememan.com")
    fake_result = TripDeleteResult(trip_id=7, ok=True)
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
        "app.routers.budget_write.resolve_scope", return_value=_fake_scope()
    ), patch("app.routers.budget_write.delete_trip", return_value=fake_result):
        mock_conn.return_value.__enter__.return_value = MagicMock()
        response = client.delete("/budget/trip", params={"trip_id": 7, "expected_updated_at": "2026-01-01T00:00:00Z"})
    assert response.status_code == 200
    assert response.json()["ok"] is True
...
def test_delete_trip_conflict_maps_to_409(client):
    from app.write_model import TripDeleteResult

    _override_auth("filler@chememan.com")
    fake_result = TripDeleteResult(trip_id=7, ok=False, error="conflict", detail="trip changed by someone else")
    with patch("app.routers.budget_write.get_fabric_conn") as mock_conn, patch(
        "app.routers.budget_write.resolve_scope", return_value=_fake_scope()
    ), patch("app.routers.budget_write.delete_trip", return_value=fake_result):
        mock_conn.return_value.__enter__.return_value = MagicMock()
