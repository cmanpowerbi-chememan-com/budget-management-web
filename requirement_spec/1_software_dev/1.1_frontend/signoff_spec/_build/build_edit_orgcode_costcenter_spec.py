...
def build_body(img_meta, rids):
    ...
    parts.append(para(
        run("ประวัติการแก้ไข: v0.3 — เพิ่มหัวข้อ \"ช่องว่างของการจับคู่ (Known coverage gaps)\" "
            "เพื่อให้ผู้ดูแลทราบว่ายังมี Cost Center / Orgcode ที่ไม่มีเจ้าของ (ตาม ADR-0007)",
            size_half_pt=18, italic=True, color="6B7280"),
        space_after=120,
    ))
    ...
    # ---- Known coverage gaps (ADR-0007) ---------------------------------
    parts.append(heading("ช่องว่างของการจับคู่ที่ควรทราบ (Known coverage gaps)"))
    parts.append(body_para(
        "ข้อมูลปัจจุบันยังมีบางส่วนที่ \"ไม่มีเจ้าของ\" — ผู้ดูแลควรทราบไว้เพื่อตรวจสอบและจับคู่เพิ่มเมื่อมีข้อมูลครบ:"
    ))
    parts.append(bullet(
        "Cost Center บางตัวยังไม่ถูกจับคู่กับ Orgcode ใดเลย (เช่น 15 CC ฝั่ง Silver ที่ยังไม่มีหน่วยงานรับผิดชอบ) "
        "→ ผู้ใช้ในหน่วยงานจะมองไม่เห็น/เข้าถึง CC นั้นในระบบงบประมาณ"
    ))
    parts.append(bullet(
        "Cost Center บางตัวมีเจ้าของแต่ไม่มีผู้กรอกงบระดับ L4 (เช่น 10AC020000 ที่ผูกกับระดับ L2 เท่านั้น) "
        "→ อาจไม่มีผู้กรอกงบประมาณของ CC นั้นโดยตรง"
    ))
    parts.append(bullet(
        "Orgcode บางตัวยังไม่ถูกจับคู่กับ Cost Center ใดเลย → สะท้อนในตัวเลข \"Unmapped\" ที่แถบสรุป (ข้อ ③)"
    ))
    parts.append(para(
        run("หมายเหตุ: ในเวอร์ชันนี้หน้าจอ \"ยังไม่\" แจ้งเตือนช่องว่างเหล่านี้โดยอัตโนมัติ — ผู้ดูแลต้องตรวจเองจากตัวเลข "
            "\"Unmapped\" และรายการการ์ด · การแจ้งเตือนช่องว่างอัตโนมัติ (gap alert) เป็นส่วนขยายในอนาคต ตาม ADR-0007",
            size_half_pt=20, italic=True, color="8C6423"),
        space_before=40, space_after=80,
    ))
