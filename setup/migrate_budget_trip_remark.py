"""
migrate_budget_trip_remark.py — one-time schema migration for the trip
`remark` field (the free-text "รายละเอียด" input at the bottom of each trip
card in Trip Manager, distinct from `purpose`/`project`).

Adds to budget.budget_trip on the live Fabric SQL DB:
  1. [remark] NVARCHAR(500) NULL   (nullable — legacy rows stay untouched)
...
Idempotent + safe:
  ...
  - NEVER run automatically (no CI hook) — jakkaritw approves, then the main
    session runs:  python setup/migrate_budget_trip_remark.py --apply

⚠ Deploy ordering: the write path is deploy-safe pre-migration (the column is
referenced ONLY when a non-null remark is sent), but `subform_read.fetch_trips`
SELECTs the column unconditionally — run this migration BEFORE deploying the
backend that carries it, or every GET /budget/trip returns 502. Same ordering
rule as `setup/migrate_budget_trip_project.py` (project) — apply BOTH
migrations before deploying a backend that carries both columns.
...
DDL_ADD_COLUMN = (
    f"ALTER TABLE {TABLE_SCHEMA}.{TABLE_NAME} ADD [{COLUMN_NAME}] NVARCHAR(500) NULL"
)
